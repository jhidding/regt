...
#include "system.hh"
#include "adhesion_traits.hh"

namespace Conan {
    using System::Header;
    using System::History;

    /*! CRTP Base of Adhesion classes.
     */
    template <class Impl>
    class AdhesionBase
    {
    public:
        constexpr static unsigned R = Impl::R;
        using RT = typename Impl::RT;
        using Traits = typename Impl::Traits;

    private:
        BoxPtr<R> box_;
        std::unique_ptr<RT> rt_;

    public:
        AdhesionBase(BoxPtr<R> box)
            : box_(box)
            , rt_(Traits::make_rt(box))
        {}

        /*! Measure of a simplex.
         */
        template <typename T>
        double measure(T &&simplex)
        {
            return Traits::measure(*rt_, std::forward<T>(simplex));
        }
    };

    template <>
    class Adhesion_base<2, NonPeriodic>
    {
    public:
    ...
    template <unsigned R, BoxTopology Topology = NonPeriodic>
    class Adhesion: public Adhesion_base<R, Topology>
    {
    public:
        using Base = Adhesion_base<R, Topology>;
