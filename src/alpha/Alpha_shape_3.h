...
public:  
  ...
  { return get_alpha_shape_vertices(it, type, get_alpha());}
   ...
   void get_filtration(Func cb)  const
   // scan  the  alpha_cell_map, alpha_min_facet_map,  alpha_min_edge_map  
   // and alpha_min_vertex in GENERAL mode 
   // only alpha_cell_map in REGULARIZED mode 
   // and output all the faces in order of alpha value of their appearing 
   ...
   { 
 	...
 	{ 
 	  ...
 	  filtration_set_management(vit, alpha_current, 
 				    facet_set, edge_set, vertex_set); 
 	  filtration_output_cb(vit->first, vit->second, cb); 
 	  vit++; 
 	} 
	 ...
	 filtration_set_management(eit, alpha_current, 
 				    facet_set, edge_set, vertex_set); 
 	filtration_output_cb(eit->first, eit->second, cb, vertex_set); 
 	eit++; 
       } 
	 ...
	 filtration_set_management(fit, alpha_current, 
 				  facet_set, edge_set, vertex_set); 
	 filtration_output_cb(fit->first, fit->second, cb,  
			   edge_set, vertex_set); 
 	fit++; 
       } 
	 ...
	 filtration_set_management(cit, alpha_current, 
 				    facet_set, edge_set, vertex_set); 
	 filtration_output_cb(cit->first, cit->second, cb,
			   facet_set, edge_set, vertex_set); 
 	cit++; 
       } 
     } 
  ...
  { 
   ...
   /*------------------------------------------------------------------\
   | Functional call-back version of filtration()                      |
   \------------------------------------------------------------------*/ 
   template<class OutputIterator> 
   void
   filtration_output_cb( const NT &alpha,  
 		     Vertex_handle vh,  
 		     OutputIterator it,  
 		     Tag_true)   const 
   {
   ...
   filtration_output_cb( const NT& alpha,  
 		     ...
 		     OutputIterator it) const 
   { 
     return filtration_output_cb(alpha, vh, it, Weighted_tag()); 
   } 
  ...
  { 
    ...
    for(int i=0; i<2; i++) { 
      ...
      if ( (get_mode()== REGULARIZED || !as->is_Gabriel())   
 	  && as->alpha_mid() == alpha  
 	  && vertex_set.find(vh[i]) == vertex_set.end() ) { 
        filtration_output_cb( alpha, vh[i], it); 
        vertex_set.insert(vh[i]); 
      } 
    } 
    it(alpha, e); 
  ...
  { 
	...
	compute_edge_status( c, i, j, as); 
      ...
      if ( (get_mode()== REGULARIZED || !as.is_Gabriel())
	   && as.alpha_mid() == alpha  
	   && edge_set.find(vhp)== edge_set.end() ) {
	filtration_output_cb( alpha, make_triple(c,i,j), it, vertex_set); 
        edge_set.insert(vhp); 
      } 
    } 
    ...
    for(int i=0; i<4; i++) { 
      ...
      if ((get_mode()== REGULARIZED || !as->is_Gabriel())
	   ...
	   && facet_set.find(std::make_pair(c->neighbor(i), 
					    this->mirror_index(c, i)))
	      == facet_set.end()) { 
        filtration_output_cb( alpha, f, it, edge_set, vertex_set); 
        facet_set.insert(f); 
      } 
    } 
